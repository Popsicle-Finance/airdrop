...
const fetchAddresses = async (provider, blockStartNumber, blockEndNumber, path) => {
    ...
    for(;i<pairLength; i+=1) {
        ...
        for(var j = blockStartNumber; j < currentBlockNumber; j+=5000) {
            ...
            console.log(pairAddress, "startNumber", j, "endNumber", z)
            let mintArray = await slpPair.queryFilter(slpPair.filters.Transfer(), j, z)
            let swapArray = await slpPair.queryFilter(slpPair.filters.Swap(), j, z)
...
fetchAddresses(fantomProvider, 2457879, undefined, "fantom.json")

fetchAddresses(fantomProvider, 2836867, 2919908, "fantom2.json")

fetchAddresses(bscProvider, 5813498, 5927198, "bsc2.json")
